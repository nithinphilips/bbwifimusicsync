...
+ BlackBerry app now support OS 5 as well as 6.
+ In the BlackBerry app and the Desktop Client the Playlists/Albums/Artists list is now searchable.
+ Simplified path for syncing.
+ Desktop client can now detect when the BlackBerry is plugged in.
+ New Installer
+ Users can now install the BlackBerry app OTA from the Server.

###Changes

 + Desktop client no longer depends on PostSharp (and should now actually work).
 + Server from the previous release did not work. Fixed.
